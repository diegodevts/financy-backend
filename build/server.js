...
var MarketPrismaRepository = class {
  ...
  addMany(data) {
    return __async(this, null, function* () {
      const market = yield prismaClient.market.createMany({ data });
      if (market) {
        return true;
      }
      return false;
  ...
  findByLocation(latitude, longitude) {
    return __async(this, null, function* () {
      const marketFounded = yield prismaClient.market.findFirst({
...
var MarketService = class {
  ...
  addMany(data) {
    return __async(this, null, function* () {
      const allMarkets = yield this.repository.findMany();
      if (allMarkets.length == 0) {
        const marketsCreated2 = yield this.repository.addMany(data);
        return marketsCreated2;
      }
      const filteredRepeatedMarkets = data.filter(
        (newMarket) => !allMarkets.some(
          (market) => market.name === newMarket.name && market.latitude === newMarket.latitude && market.longitude === newMarket.longitude
        )
      );
      const marketsCreated = yield this.repository.addMany(
  ...
  findByLocation(latitude, longitude) {
    return __async(this, null, function* () {
      const market = yield this.repository.findByLocation(latitude, longitude);
      return market;
    });
  }
};
...
var MarketController = class {
  ...
  addMany(request, response) {
    return __async(this, null, function* () {
      try {
        const data = request.body;
        const hasMarketsCreated = yield this.service.addMany(data);
...
var auth3 = new Auth();
endpoint3.post("/add", auth3.execute, (request, response) => {
...
endpoint3.get("/all", auth3.execute, (request, response) => {
...
// src/routes/product/routes.ts
var endpoint4 = (0, import_express4.Router)();
var auth4 = new Auth();
...
// src/routes/refresh-token/routes.ts
var endpoint5 = (0, import_express5.Router)();
endpoint5.get(
...
var routes_default5 = endpoint5;

// src/routes.ts
var routes = (0, import_express6.Router)();
routes.use("/user", routes_default);
routes.use("/expense", routes_default2);
routes.use("/market", routes_default3);
routes.use("/product", routes_default4);
routes.use("", routes_default5);
